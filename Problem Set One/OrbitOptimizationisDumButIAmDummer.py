...
#####################
"""
def get_acc(p,chosenDate):
    earthPos = e.position('earthmoon', chosenDate) - e.position('moon', chosenDate) * e.earth_share
    earthPos = vector(earthPos[0][0], earthPos[1][0], earthPos[2][0])
    return -1. * sunMu * p / (mag(p)**3)# - 1. * earthMu * (earthPos - p) / (mag(earthPos - p)**3)

def get_first(position, velocity, timestep,time):
    acc = get_acc(position,time)
    first = position+velocity*timestep+0.5*acc*timestep**2
    return first

r1=get_first(r2,r2dotty,dtau,JD[1])
print r1
...
def get_pos(time, pos1, pos2,tau,dtau,angromariam):
    if time>0:
        pos2 = get_first(r2,r2dotty,dtau,angromariam)
        while tau<time:
            accel = get_acc(pos1,angromariam)
            pos3=2*pos2-pos1+accel*dtau**2
            pos1=pos2
            pos2=pos3
            tau= tau + dtau
        return pos2
...
print r2_dot
"""
  <0.665882, -1.49303, 0.137247>
VELOCITY:  <-0.1168, -0.0462224, 0.461564>
CHI SQUARED:  56.2929229014
1.65450121655% PASSED
ITERATION:  2056
FAILED
POSITION:  <0.665882, -1.49303, 0.137247>
VELOCITY:  <-0.1168, -0.

"""
r2 = vector(0.676618, -1.54539, 0.170366)
r2_dot = vector(0.0867856, 0.0245477, 0.283733)
